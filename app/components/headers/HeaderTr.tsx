...
const NAV: NavGroup[] = [
  ...
  },
  {
    ...
    href: "/tr/projects",
        items: [
      { label: "Lagoon Verde", href: "/tr/lagoon-verde" },
      { label: "La Joya Perla II", href: "/tr/perla-ii" },
            { label: "La Joya Perla", href: "/tr/perla" },


      { label: "La Joya", href: "/tr/la-joya" },
      { label: "Mariachi Beach Club", href: "/tr/mariachi" },
  ...
  },
  {
    label: "Hakkımızda",
    href: "/tr/about",
    items: [{ label: "Ekibimiz", href: "/tr/team" }],
  },
...
const handleSwitch = (target: "tr" | "en") => {
  if (target === locale) return setOpenLang(false);
  router.push(withLocale(target, pathname)); // `pathname` is your clean path from useNormalizedPathname()
  setOpenLang(false);
};
...
// build a locale-prefixed path from a clean path (no locale)
function withLocale(locale: "tr" | "en", cleanPath: string) {
  return locale === "tr"
  ...
  <AnimatePresence>
    {openLang && (
      <motion.div
        ...
        className="absolute right-0 mt-2 min-w-[140px] rounded-xl overflow-hidden shadow-lg z-[60]"
        style={{
          // match your glass styles to header theme
          background: onTransparent
            ? "rgba(20,21,23,0.78)"
            : "linear-gradient(180deg, rgba(255,255,255,0.98), rgba(255,255,255,0.94))",
          border: onTransparent
      ...
      >
        <button
          ...
          className={[
            ...
            "hover:bg-white/10",
            onTransparent ? "text-white/95" : "text-black/80"
          ].join(" ")}
        >
          Türkçe
        </button>
        <button
          ...
          className={[
            "w-full text-left px-3 py-2 text-sm",
            "hover:bg-white/10 border-t",
            onTransparent ? "text-white/95 border-white/10" : "text-black/80 border-black/10"
          ].join(" ")}
        >
          English
        </button>
      ...
      <AnimatePresence>
        {menuOpen && (
          <>
            ...
            {/* Mobile drawer — Language switcher (dropdown) */}
<div className="px-5 py-3 flex items-center justify-between">
  ...
  <div className="relative" ref={langRef}>
    ...
    <button
      ...
      aria-haspopup="menu"
      aria-expanded={openLang}
      className="text-xs px-2.5 py-1.5 rounded-lg bg-white/10 border border-white/20 hover:bg-white/20 inline-flex items-center gap-1"
    >
      {locale.toUpperCase()}
      <svg width="10" height="10" viewBox="0 0 20 20" className="ml-1" aria-hidden="true">
        <path fill="currentColor" d="M5 7l5 6 5-6z" />
      </svg>
    </button>
    ...
    <AnimatePresence>
      {openLang && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 6, scale: 0.98 }}
          transition={{ duration: 0.16, ease: [0.22, 1, 0.36, 1] }}
          className="absolute right-0 mt-2 min-w-[140px] rounded-xl overflow-hidden shadow-lg z-[10002]"
          style={{
            background: "rgba(20,21,23,0.90)",
        ...
        >
          <button
            onClick={() => handleSwitch("tr")}
            role="menuitem"
            className="w-full text-left px-3 py-2 text-sm hover:bg-white/10 text-white/95"
          >
            Türkçe
          </button>
          <button
            onClick={() => handleSwitch("en")}
            role="menuitem"
            className="w-full text-left px-3 py-2 text-sm hover:bg-white/10 text-white/95 border-t border-white/10"
          >
            English
          </button>
        </motion.div>
